...
void light_core_port_worker_launch(void (*worker_fn)(void))
{
        ...
        //   core 1 is reset before launching it, reversing an earlier deliberate choice not to.
        // The reasoning then was that multicore_reset_core1() corrupts a debugger's view of core
        // 1 once a probe has examined both DAPs (raspberrypi/pico-sdk#2249), and that it "isn't
        // needed for a real, undebugged boot".
        //
        ...
        // it waits on a FIFO handshake from a core busy in the previous launch's worker loop,
        // which will never answer, and the SDK call has no timeout. Any warm restart of core 0
        // alone reaches that state -- a debugger reset, a re-init, a second light_stream_setup().
        //
        //   diagnosed on RP2350 as core 0 stuck in multicore_fifo_wready() from
        // multicore_launch_core1_raw(), with no output past "loaded N static output streams".
        // That reads as a dead board and says nothing about the cause; a silent permanent hang
        // is a worse failure than a debugger losing sight of a core.
        //
        //   NOTE: verified on RP2350 hardware, where the debugger kept full view of core 1 after
        // this change (no "Failed to select multidrop"). NOT verified on RP2040 -- no board to
        // hand -- so if #2249 does bite here, it bites in the direction of a debugging
        // inconvenience rather than the hang this removes
