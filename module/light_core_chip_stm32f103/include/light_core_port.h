...
// see the F411 port's light_core_port.h for why this is defined here rather than picked up
// from a vendor header
#define __packed_aligned __attribute__ ((packed, aligned(4)))

// Cortex-M3 has LDREX/STREX too, so the same reasoning as the F411 port applies: a plain
// uint32_t guarded by the registry's critical section, not a divergent lock-free path
typedef uint32_t light_ref_t;

//   see the F411 port's light_core_port.h for the full rationale -- unchanged here: no OS, one
// core, PRIMASK saved/restored so nesting is safe.
...
// brings up the USART console -- see console.c. Called from light_platform_init() before
// anything can log
