/*
 *  light_core_arm32_m.c
 *  light_core arch-specific code for generic ARM Cortex-M targets
 * 
...
{
        // we need to check both that the source string is not empty, and that
        // the name field, once generated, is itself not empty
        if(!format || !format[0]) {
